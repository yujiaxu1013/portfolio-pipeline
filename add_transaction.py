...
def normalize_ticker(raw):
    """統一標的格式:自動補上 .TW(若使用者忘了加)"""
    raw = raw.upper().strip()
    if not raw.endswith(".TW"):
        raw = raw + ".TW"
    return raw

def main():
    print("=== 記一筆交易 ===")
    ticker   = normalize_ticker(ask("標的代號", default="0050.TW"))
    ...
    print(f"\n即將寫入:{ticker} | {txn_date} | buy {shares} 股 @ {price} | 手續費 {fee}")
    ...
    """, (ticker, txn_date, shares, price, fee, note))
    conn.commit()

    # 立刻印出最新持倉對帳
    cur.execute("SELECT ticker, total_shares, net_cost FROM current_holdings")
    for tk, total_shares, net_cost in cur.fetchall():
        print(f"\n最新持倉:{tk} 共 {total_shares} 股,總成本 {net_cost:,.2f}")
